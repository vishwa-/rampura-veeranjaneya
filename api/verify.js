...
import { readJson, sendJson, sendErr, hmacSha256Hex, timingSafeEqualHex } from "./_lib/util.js";
...
export default async function handler(req, res) {
  ...
  if (!timingSafeEqualHex(expected, razorpay_signature)) {
    return sendErr(res, "bad_signature", 400);
  }
  ...
  if (error || !data?.found) return sendErr(res, "not_found", 404);
