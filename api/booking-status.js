...
import { query, sendJson, sendErr } from "./_lib/util.js";
...
export default async function handler(req, res) {
  const orderId = query(req).get("order_id") || "";
  if (!/^order_[A-Za-z0-9]+$/.test(orderId)) return sendErr(res, "bad_request");
  ...
  const { data, error } = await db()
    ...
    .single();
  if (error || !data) return sendErr(res, "not_found", 404);
